...
- 设备工作区默认投屏：右侧 scrcpy H.264 WebSocket 实时预览（WebCodecs、ws-scrcpy 协议）；设备端 `4.0-ws1` 自编译 server，缺失时后端自动 Gradle 编译
...
- Device workspace default cast: scrcpy H.264 WebSocket preview (WebCodecs, ws-scrcpy protocol); device runs custom `4.0-ws1` server, auto-built by backend when missing
...
- Sync frontend and backend versions to `0.6.0`
