...
- 设备工作区默认投屏：右侧 scrcpy H.264 WebSocket 实时预览（WebCodecs、ws-scrcpy 协议）；设备端自编译 server（`4.0`），缺失时后端自动 Gradle 编译；启动前自动清理残留进程避免 8886 端口占用；修复网页黑屏（Annex-B 连续解码与 WS 代理竞态）
...
- Device workspace default cast: scrcpy H.264 WebSocket preview (WebCodecs, ws-scrcpy protocol); device runs custom `4.0` server (auto-built by backend when missing); auto-cleans leftover device server to avoid port 8886 conflicts; fixes web black screen (continuous Annex-B decode and WS proxy race)
...
- Sync frontend and backend versions to `0.6.2`
