...
#include "cli.h"
#include "cloud_phone/cloud_phone.h"
#include "events.h"
#include "options.h"
#include "scrcpy.h"
#ifdef HAVE_USB
...
main_scrcpy(int argc, char *argv[]) {
    ...
    if (sc_cloud_phone_should_print_capabilities(argc, argv)) {
        sc_cloud_phone_print_capabilities();
    ...
    if (!sc_cloud_phone_prepare_argv(argc, argv, &effective_argc, &effective_argv,
                                     &allocated_argv)) {
        ret = SCRCPY_EXIT_FAILURE;
        goto end;
    }

    if (!scrcpy_parse_args(&args, effective_argc, effective_argv)) {
        ret = SCRCPY_EXIT_FAILURE;
        goto release_argv;
    }
...
release_argv:
    sc_cloud_phone_release_argv(allocated_argv, effective_argc);
